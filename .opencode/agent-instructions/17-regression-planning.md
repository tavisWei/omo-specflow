...
- **等待与轮询约束**: 任何等待、轮询、重试、循环搜索型测试步骤，都必须写明 timeout、最大尝试次数、最大持续时间或明确退出条件；禁止无限等待和无限循环。

### 第三步：执行与证据收集 (Execution & Evidence)
- 记录每个测试项的通过/失败状态。
- 对于关键路径，必须捕获执行证据（如 `npm test` 的输出摘要）。
- 若出现等待超时或达到轮询/循环上限，必须显式记录为失败原因，不允许静默跳过或继续无限重试。
...
- [ ] 测试证据是否足以证明功能正常？
- [ ] 是否验证了错误处理逻辑？
- [ ] 所有等待步骤是否都声明了 timeout 与超时失败退出？
- [ ] 所有轮询 / 重试 / 循环搜索步骤是否都声明了 maxAttempts、maxDuration 或其他显式退出条件？
...
- 禁止编写没有 timeout 的等待步骤。
- 禁止编写没有上限或退出条件的轮询、重试、循环搜索步骤。
