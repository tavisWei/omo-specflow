...
- 修复涉及多个模块的复杂 Bug。
- 升级核心依赖或中间件。
- 日常 QA / 需求对齐后，需要先判断“已实现 / 部分实现 / 偏离需求 / 未实现”，再进入增量迭代。
...
### 第零步：QA 先导差异识别 (QA-First Gap Classification)
> 仅在本次迭代起点来自日常 QA、需求复核或原型对齐检查时执行。

1. **收集真源**: 汇总原始 PRD、原型图、页面说明、验收口径和当前实现。
2. **输出差异结论**: 先将每个检查点归类为 `已实现 / 部分实现 / 偏离需求 / 未实现`。
3. **只把有差异项带入后续步骤**: 已实现项不进入 Delta Spec 和任务分解。
4. **显式记录回归影响**: 对“部分实现 / 偏离需求 / 未实现”项标记受影响模块和回归范围。
...
- [ ] 若起点来自日常 QA，是否已先输出“已实现 / 部分实现 / 偏离需求 / 未实现”的差异结论？
- [ ] 是否已扫描所有受影响的调用链路？
- [ ] 是否已识别出所有潜在的破坏性变更？
- [ ] 增量 Spec 是否与现有文档冲突？
- [ ] 回归计划是否覆盖了所有高风险区域？
