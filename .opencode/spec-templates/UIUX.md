...
### 1.0 风格锚点 | Style Anchors

<!-- AGENT: 如果用户提供了现有原型 / 截图 / 页面稿，先列出 2-5 个风格锚点页面或组件，新增页面必须优先继承这些锚点。若无现成材料，明确写“无外部风格锚点，按本文档设计系统推导”。 -->

- **参考页面 / 组件**: <!-- AGENT: 如 "登录页、仪表盘、列表页、主表单弹窗" -->
- **必须保持一致的元素**: <!-- AGENT: 如 "色彩层级、卡片圆角、表格样式、筛选栏布局、按钮层级" -->
...
<!-- AGENT: 从 PRD.md 2.2 核心场景推导页面列表。若需求、菜单、页面说明提到某功能但现有原型未覆盖，也必须补入页面清单，并在后文标记其来源。 -->
...
### 2.1 页面来源标记 | Page Source Markers

<!-- AGENT: 对新增补全页面标记来源，避免“页面是自己补的但无需求依据”。 -->

| 页面 Page | 来源 Source | 备注 Note |
|---|---|---|
| <!-- AGENT: 页面名 --> | <!-- AGENT: 原型 / 菜单 / PRD / 页面说明 --> | <!-- AGENT: 如 "原型缺失，按菜单功能补全" --> |
